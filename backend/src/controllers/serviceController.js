...
const createService = async (req, res) => {
  try {
    ...
    if (!service_name || !price) {
      return res
        .status(400)
        .json({ message: "Please provide service name and price" });
    }
...
const updateService = async (req, res) => {
  try {
    ...
    if (service.length === 0) {
      ...
      return res
        .status(403)
        .json({ message: "Access denied,only admin can update service" });
    }
    await pool.query(
...
const deleteService = async (req, res) => {
  try {
    ...
    if (service.length === 0) {
      ...
      return res
        .status(403)
        .json({ message: "Access denied, only admin can delete service" });
    }
    ...
    await pool.query("DELETE FROM time_slots WHERE service_id = ?", [
      service_id,
    ]);
...
const getTimeSlots = async (req, res) => {
  try {
    ...
    let query =
      "SELECT * FROM time_slots WHERE service_id = ? ORDER BY start_time";
    ...
    if (isAdmin) {
      ...
      if (service.length === 0) {
        return res
          .status(404)
          .json({
            message: "you do not have permission to view these time slots",
          });
      }
    }
...
const getAvilableSlots = async (req, res) => {
  try {
    ...
    const avilable = allSlots.filter(
      (slot) => !bookedSlotIds.has(slot.slot_id),
    );
  ...
  try {
    ...
    if (service.length === 0) {
      return res
        ...
        .json({
          message:
            "You do not have permission to create time slots for this service",
        });
    }
...
//Update time slot status (Admin only - for own slots)
const updateTimeSlot = async (req, res) => {
  try {
    ...
    if (!status) {
      return res
        .status(400)
        .json({ message: "Please provide status to update" });
    }

    const validateStatus = ["available", "unavailable"];
    if (!validateStatus.includes(status)) {
      return res
        .status(400)
        .json({
          message: "Invalid status value , must be available or unavailable",
        });
    }

    // Check if slot belongs to admin's
    ...
    if (slot.length === 0) {
      return res
        .status(404)
        .json({
          message: "Time slot not found , You must have timeslot to update it ",
        });
    }
